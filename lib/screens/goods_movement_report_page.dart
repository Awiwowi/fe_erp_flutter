...
class GoodsMovementReportPage extends StatefulWidget {
  ...
  @override
  State<GoodsMovementReportPage> createState() =>
      _GoodsMovementReportPageState();
}

class _GoodsMovementReportPageState extends State<GoodsMovementReportPage>
  ...
  Future<void> _fetchData() async {
    ...
    var incomingRes = await DataService().getIncomingGoodsReport(
      startDate: startStr,
      endDate: endStr,
      search: searchStr,
    );
    var outgoingRes = await DataService().getOutgoingGoodsReport(
      startDate: startStr,
      endDate: endStr,
      search: searchStr,
    );
    ...
    if (mounted) {
      setState(() {
        ...
        _incomingGrandTotal =
            double.tryParse(
              incomingRes?['meta']?['grand_total_qty']?.toString() ?? '0',
            ) ??
            0;
        ...
        _outgoingGrandTotal =
            double.tryParse(
              outgoingRes?['meta']?['grand_total_qty']?.toString() ?? '0',
            ) ??
            0;
            ...
            itemBuilder: (context, index) {
              ...
              return ExpansionTile(
                ...
                children: categories.entries.map((entry) {
                  String categoryName =
                      entry.key; // 'Product' atau 'RawMaterial'
                  var categoryData = entry.value;
                  List<dynamic> items = categoryData['items'] ?? [];
                  String subTotal =
                      categoryData['sub_total_qty']?.toString() ?? '0';
                        ...
                        SingleChildScrollView(
                          ...
                          child: DataTable(
                            ...
                            columns: [
                              ...
                              if (!isIncoming)
                                const DataColumn(
                                  label: Text("Penginput"),
                                ), // Di outgoing ada nama_pengambil
                              const DataColumn(
                                label: Text("Qty", textAlign: TextAlign.right),
                              ),
                            ],
                            rows: items.map((item) {
                              return DataRow(
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                ...
                // PERBAIKAN: Diubah menjadi Column agar tanggal turun ke bawah Search Bar
                Column(
                  children: [
                    TextField(
                    ...
                    const SizedBox(
                      height: 15,
                    ), // Jarak antara search dan tanggal
                    InkWell(
            ...
            child: TabBar(
              ...
              tabs: const [
                Tab(
                  ...
                  text: "Log Barang Masuk",
                ),
                Tab(
                  icon: Icon(Icons.arrow_upward, color: Colors.red),
                  text: "Log Barang Keluar",
                ),
              ],
            ),
          ),
